...
void registerPub(ros::NodeHandle &n) {
  ...
  pub_initial_pose = n.advertise<nav_msgs::Odometry>("camera_pose", 100);
...
void pubCameraPose(const Estimator &estimator, const std_msgs::Header &header) {
  ...
  // xjl
    ...
    Matrix3d map_yaw = Utility::ypr2R(Eigen::Vector3d{MAP_YAW, MAP_PITCH, MAP_ROLL});
    ...
    odometry.pose.pose.orientation.z = R.z();
    odometry.pose.pose.orientation.w = R.w();
    pub_initial_pose.publish(odometry);
    // estimator.initGSFlag == false;
    cameraposevisual.reset();
    cameraposevisual.add_pose(P, R);
    // if (STEREO) {
    //   Vector3d P = estimator.Ps[i] + estimator.Rs[i] * estimator.tic[1];
...
// xjl
void pubGT(const Estimator &estimator, const std_msgs::Header &header) {
    // 确保 groundtruth 是可以被修改的（去掉 const 限制）
    for (const nav_msgs::Odometry &point : estimator.groundtruth) {
        if (point.header.stamp.toSec() <= header.stamp.toSec()) {
            pub_gt_pose.publish(point);
        } else {
            break; // 提前结束循环，后续点不满足条件
        }
    }
}
