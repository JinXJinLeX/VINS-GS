...
void Estimator::processMeasurements() {
  groundtruth = loadOdometryFromTumFile(GROUNDTRUTH);
  while (1) {
    ...
    if (!featureBuf.empty()) {
      ...
      // xjl
      // if (solver_flag == INITIAL && initGSFlag == true) {
      //   pubINITIALPOSE(*this, header);
      //   initGSFlag = false;
      // }
      pubGT(*this, header);
      ...
      while (!tempQueue.empty()) {
        if (tempQueue.front().time < Headers[i])
          tempQueue.pop();
                 ...
                 abs(tempQueue.front().time - Headers[i]) < 0.01) {
          GStime = tempQueue.front().time;
          Eigen::Quaterniond q(tempQueue.front().R);
          Eigen::Vector3d t(tempQueue.front().T);
          render_Pose[i][0] = t.x();
          render_Pose[i][1] = t.y();
          render_Pose[i][2] = t.z();
          render_Pose[i][3] = q.x();
          ...
          // printf("\033[1;92mGS_Pose- i: %d,  size: %d
...
void Estimator::processImage(
  ...
  if (solver_flag == INITIAL) {
    // monocular + IMU initilization + GS
    if (!STEREO && USE_IMU) {
  ...
  } else {  // 初始化成功之后状态机flag不再是INITIAL
    TicToc t_solve;
    if (!USE_IMU)
      f_manager.initFramePoseByPnP(frame_count, Ps, Rs, tic,
                                   ric);  // 如果没有IMU就用pnp解个位姿
    f_manager.triangulate(frame_count, Ps, Rs, tic,
                          ric);  // 有IMU就直接三角化了
    optimization();              // 优化
    // GSmapoptimization();
    set<int> removeIndex;
    outliersRejection(removeIndex);  // 移除外点，可以在这里做质量控制的修改
...
bool Estimator::visualInitialAlign() {
  ...
  VectorXd x;
  // solve scale
  bool result = VisualIMUAlignment(all_image_frame, Bgs, g, x, GS_pro);
  if (!result) {
...
void Estimator::double2vector() {
  ...
  if (USE_IMU) {
    ...
    // xjl
    if (GAUSSIAN_MAP) {
      if (count > int(float(WINDOW_SIZE) * 0.5)) {
        for (int i = 0; i <= WINDOW_SIZE; i++) {
...
// xjl
std::vector<nav_msgs::Odometry> Estimator::loadOdometryFromTumFile(
