...
int SuperGlue::matching_points(Eigen::Matrix<double, 259, Eigen::Dynamic>& features0,
  ...
  for(size_t i = 0; i < indices0.size(); i++){
    if(indices0(i) < indices1.size() && indices0(i) >= 0 && indices1(indices0(i)) == i){
      double avg_score = (mscores0[i] + mscores1[indices0[i]]) / 2.0;
      const double score_threshold = 0.4; // 提高阈值
  ...
  if(outlier_rejection){
    ...
    // std::cout << "check glue ransac " <<  std::endl;
    // std::cout << "points0: " << points0.size() << std::endl;
    // std::cout << "points1: " << points1.size() << std::endl;
    if (points0.size() > 10 && points1.size()>10) {
        cv::findFundamentalMat(points0, points1, cv::FM_RANSAC, 2.0, 0.9, inliers);
