...
void gaussian_callback(
  ...
  sensor_msgs::ImageConstPtr depth_msg =
      boost::make_shared<sensor_msgs::Image>(render_msg->DEPTHimage);
  cv::Mat depth_image = cv_bridge::toCvShare(depth_msg, "32FC1")->image;
  sensor_msgs::ImageConstPtr conf_msg =
      boost::make_shared<sensor_msgs::Image>(render_msg->CONFimage);
  cv::Mat conf_image = cv_bridge::toCvShare(conf_msg, "32FC1")->image;

  double position[7];
  position[0] = pose.position.x;
  position[1] = pose.position.y;
  position[2] = pose.position.z;
  position[3] = pose.orientation.x;
  position[4] = pose.orientation.y;
...
int main(int argc, char **argv) {
  ...
  // message_filters::Subscriber<sensor_msgs::Image> depth_sub(n,
  // "/rendered_depth", 50);
  // // 设置同步策略（使用 ApproximateTime 策略进行近似时间同步）
  // typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::Image,
  // sensor_msgs::Image> SyncPolicy; message_filters::Synchronizer<SyncPolicy>
  // sync(SyncPolicy(10), rgb_sub, depth_sub);
  // // 设置同步后的回调函数
  // sync.registerCallback(boost::bind(&gaussian_callback, _1, _2));
  ros::Subscriber sub_render = n.subscribe("/render", 100, gaussian_callback);
