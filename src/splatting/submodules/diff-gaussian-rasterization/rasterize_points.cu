	...
	const bool debug)
{
  ...
  torch::Tensor confidence = torch::full({P}, 0, float_opts);
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward(
		...
		sh_conf.contiguous().data<float>(),
		confidence.contiguous().data<float>(),
		out_confidence.contiguous().data<float>(),
		debug);
  }
  return std::make_tuple(rendered, out_color, radii, geomBuffer, binningBuffer, imgBuffer, out_invdepth, out_confidence);
}
...
//   constexpr int64_t kPrint = 20;
//   int64_t actual = std::min<int64_t>(kPrint, dL_dcolors.size(0));

//   auto g_col = dL_dcolors.slice(0, 0, actual).to(torch::kCPU); 
...
//     printf("[%lld] color=", i);
//     for (int64_t c = 0; c < g_col.size(1); ++c) printf("% .6e ", g_col[i][c].item<float>());
//     printf(" scale=");
